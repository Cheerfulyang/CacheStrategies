...
from icarus.tools import traces
    ...
    def __iter__(self):
        ...
        while req_counter < self.n_warmup + self.n_measured:
            ...
            if k != 0:
                ...
                receiver = random.choice([v for v in self.receivers if content%64 == v%64])
            log = (req_counter >= self.n_warmup)
            event = {'receiver': receiver, 'content': content, 'log': log}
            yield (t_event, event)
            req_counter += 1
...
@register_workload('YOUTUBE_TRACE')
class YOUTUBE_TRACE(object):
    ...
    """
    def __init__(self, topology, n_contents, alpha, beta=0, rate=1.0,
                    n_warmup=10 ** 3, n_measured=4 * 10 ** 5, seed=None, **kwargs):
        if alpha < 0:
            raise ValueError('alpha must be positive')
        if beta < 0:
            raise ValueError('beta must be positive')
    ...
    def __iter__(self):
        ...
        for n in my_parse_youtube_umass(mypath):
            t_event += (random.expovariate(self.rate))
            if self.beta == 0:
                receiver = self.receivers[int(n['client_addr'])%len(self.receivers)]
            else:
                receiver = self.receivers[self.receiver_dist.rv() - 1]
            content = n['video_id']
            log = (req_counter >= self.n_warmup)
