...
__all__ = [
    ...
    'edge_cache_placement',
    'core_cache_placement',
...
@register_cache_placement('EDGE')
def edge_cache_placement(topology, cache_budget, **kwargs):
    """Places cache budget uniformly across cache nodes.
    ...
    """
    icr_candidates = topology.graph['icr_candidates']
    cache_size = iround(cache_budget / 9)
    for v in icr_candidates:
        if topology.node[v]['depth'] == 2: 
            topology.node[v]['stack'][1]['cache_size'] = cache_size
            

@register_cache_placement('CORE')
...
@register_cache_placement('HYBRID')
def hybrid_cache_placement(topology, cache_budget, **kwargs):
    """Places cache budget hybridly across BBU and RRH cache.
    ...
    for v in icr_candidates:
        if topology.node[v]['depth'] == 1: 
            topology.node[v]['stack'][1]['cache_size'] = cache_size/2  
        if topology.node[v]['depth'] == 2: 
            topology.node[v]['stack'][1]['cache_size'] = cache_size/6          
